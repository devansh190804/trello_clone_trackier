...
const UpdateProjectForm = ({ project, onSuccess }) => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      await sendApiRequest('put', `/api/project/${project._id}`, {
        title: data.title,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="w-80">
      <h2 className="text-lg font-semibold mb-4" style={{ color: '#172b4d' }}>Update Project</h2>
      <div className="mb-4">
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? <span className="animate-spin h-5 w-5 border-4 border-white border-t-transparent rounded-full"></span> : 'Update Project'}
      </button>
    </form>
  );
};
